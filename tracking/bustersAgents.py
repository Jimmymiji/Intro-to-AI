...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for d in livingGhostPositionDistributions:
            ...
            if not nearest_pos:
                nearest_pos = pos
            elif self.distancer.getDistance(pos, pacmanPosition) < self.distancer.getDistance(nearest_pos, pacmanPosition):
                nearest_pos = pos
        min_dist = 99999999999
        min_action = None
        for a in legal:
               next_pos = Actions.getSuccessor(pacmanPosition, a)
               if self.distancer.getDistance(nearest_pos,next_pos) <  min_dist:
                   min_dist = self.distancer.getDistance(nearest_pos,next_pos)
                   min_action = a
        return min_action
