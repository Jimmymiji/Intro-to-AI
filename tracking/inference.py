...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        self.particles = list()
        num_of_pos = len(self.legalPositions)
        num_of_p = self.numParticles / num_of_pos

        for p in self.legalPositions:
            for i in range(num_of_p):
                self.particles.append(p)
        
        for i in range(self.numParticles - num_of_p*num_of_pos):
            idx = random.randint(0,num_of_pos-1)
            self.particles.append(self.legalPositions[idx])
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for i in range(self.numParticles):
                ps.append(self.getJailPosition())
            self.particles = ps
        else:
            weights = util.Counter()
            for p in self.particles:
                true_dist = util.manhattanDistance(p,pacmanPosition)
                weights[p] += emissionModel[true_dist]
            if weights.totalCount() == 0:
                ...
                for pos,prob in weights.items():
                    num = int(prob * self.numParticles)
                    for i in range(num):
                        ps.append(pos)
                self.particles = ps
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.particles:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            new_pos = util.sample(newPosDist)
            ps.append(new_pos)
        self.particles = ps
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = list()
        a = list(itertools.product(self.legalPositions,repeat=self.numGhosts))
        random.shuffle(a)
        ...
        for p in self.particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] == None:
                    ...
                    true_dist = util.manhattanDistance(p[i],pacmanPosition)
                    prob *= emissionModels[i][true_dist]
            weights[p] += prob
        if weights.totalCount() == 0:
            self.initializeParticles()
        else:
